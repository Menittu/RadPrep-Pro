    ...
    <div className="space-y-8 animate-fadeIn pb-12">
      <div className="relative overflow-hidden bg-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl border border-white/5">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="hidden lg:block bg-white/5 p-8 rounded-[3rem] backdrop-blur-xl border border-white/10 shadow-2xl">
      ...
      {activeSession && !activeSession.isCompleted && (
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border-2 border-amber-500/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-amber-500/10">
          ...
          <button onClick={resumeTest} className="w-full md:w-auto px-10 py-4 bg-amber-500 text-white font-black rounded-2xl transition-all shadow-lg hover:bg-amber-600 flex items-center justify-center gap-3">
      ...
      {chapter ? (
        <div className="space-y-6 animate-slideUp">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <ModeCard title="Practice" desc="Instant feedback and in-depth logic for every question. Ideal for initial learning." icon={<PlayCircle size={36} className="text-emerald-500" />} accentColor="emerald" onClick={() => startTest(TestMode.PRACTICE)} />
            <ModeCard title="Mock Exam" desc="Timed pressure. No feedback until submission. Perfect for testing exam readiness." icon={<Target size={36} className="text-rose-500" />} accentColor="rose" onClick={() => startTest(TestMode.MOCK)} />
          </div>
        </div>
      ) : (
        <div className="p-20 text-center border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-[3rem] bg-white dark:bg-slate-900/50">
          <div className="mx-auto w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-full flex items-center justify-center text-indigo-500 mb-8">
            <BookOpen size={48} strokeWidth={1.5} />
          </div>
          <h3 className="text-3xl font-black text-slate-400 mb-3 tracking-tight">Select Chapter</h3>
          <p className="text-slate-400 dark:text-slate-500 font-medium max-w-xs mx-auto">Pick a radiotherapy chapter from the sidebar menu to begin your mastery journey.</p>
        </div>
      )}
    </div>
  );
...
const StatCard = ({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) => (
  <div className="bg-white/5 px-6 py-3 rounded-2xl border border-white/10 flex items-center gap-4 hover:bg-white/10 transition-colors">
...
const ModeCard = ({ title, desc, icon, onClick, accentColor }: { title: string, desc: string, icon: React.ReactNode, onClick: () => void, accentColor: string }) => {
  ...
  return (
    <button onClick={onClick} className={`group text-left p-10 bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-transparent transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 flex flex-col h-full ${accentClasses} shadow-sm border-gray-100 dark:border-slate-800`}>
      <div className="mb-8 bg-slate-50 dark:bg-slate-800/50 w-20 h-20 rounded-3xl flex items-center justify-center group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <h3 className="text-3xl font-black mb-4 text-slate-800 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{title}</h3>
